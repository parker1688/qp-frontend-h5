<template>
  <view class="container-box">
    <uv-navbar placeholder bgColor="#1c1c1c" @leftClick="pagesRoute" leftIconColor="#fff" :titleStyle="{color:'#fff'}" title="充值记录">
...
<!--            <view v-if="item.status==3 || item.status==2">{{item.pay_time }}</view>-->
<!--            <view v-else>{{item.create_time }}</view>-->
          ...
          <view class="u-flex u-flex-between">
            ...
            <view class="u-flex-xy-center" v-else-if="item.status==1">待处理</view>
    ...
    <dayPopup ref="dayPopupRef" :list="typeList"  @getData="getFundingType" notTetx="重置" title="充值类型"/>
  </view>
</template>
...
import { walletListDeposit } from "@/api/money";
...
const { pager,resetPageNum, getLists ,resetPage} = usePaging({
  fetchFun: walletListDeposit,
  params: formData,
});

//状态	1:待处理 2:拒绝失败 3:已通过 7:待支付
...
//支付渠道类型 1:微信 2:银行卡 3:支付宝 4:钱包 5:数字人民币 6:数字货币 20充值
const zfTypeList =ref([
  {name:'微信',type:withdrawEnum.WX},
  {name:'银行卡',type:withdrawEnum.CARD},
  {name:'支付宝',type:withdrawEnum.ZFB},
  {name:'钱包',type:withdrawEnum.PURSR},
  {name:'数字人民币',type:withdrawEnum.NUMRMB},
  {name:'数字货币',type:withdrawEnum.NUMHB},
  {name:'人工充值',type:withdrawEnum.RGCZ},
...
const getStartAtEndAt=(type:number)=>{
  ...
  // formData.start_time=res.startAt
  // formData.end_time=res.endAt
}

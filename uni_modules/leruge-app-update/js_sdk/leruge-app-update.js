...
function showDialog(updateInfo) {
	...
	let descriptionList =[]
	if(updateInfo.updateContent){
		descriptionList = drawtext(updateInfo.updateContent, viewContentWidth)
	}
	//const descriptionList = drawtext((updateInfo.updateContent || '发现新版本'), viewContentWidth)
	let popupViewHeight = 80 + 20 + 20 + 90 + 10
	let popupViewContentList = [{
			src: '/uni_modules/leruge-app-update/static/ic_ar.png',
			id: "logo",
			tag: "img",
			position: {
				top: "0px",
...
// 下载流程
function downloadNew(updateInfo) {
	let platform = updateInfo.platform || 'android'
	if (updateInfo.downUrl) {
		getDownloadnew(updateInfo)
	} else {
		plus.nativeUI.alert('下载地址无效')
	}
}
...
// 从服务器下载应用资源包（wgt文件）
const getDownloadnew = function(data) {
	let dtask
	dtask = plus.downloader.createDownload(data.downUrl, {
		filename: "_doc/update/"
	}, function(download, status) {
	...
	dtask.addEventListener("statechanged", function(task, status) {
		switch (task.state) {
			...
			case 3:
				const progress = parseInt(task.downloadedSize / task.totalSize * 100);
				let data={
					downloadedSize:(task.downloadedSize / 1024 / 1024).toFixed(2),
					totalSize:(task.totalSize / 1024/ 1024).toFixed(2),
					progress :progress
				}
				userStore().setDownVersion(data)
				break;
		}
	})
}
